package cart;

import java.io.IOException;
import java.util.List;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;

import dao.CartInfoDao;
import vo.CartJoinProduct;
import vo.MemberInfo;

@WebServlet("/cart/list")
public class CartListController extends HttpServlet {
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		HttpSession session = request.getSession();
		MemberInfo memberInfo = (MemberInfo) session.getAttribute("loginUserInfo");
		
		int member_userIdx = memberInfo.getUserIdx();
		
		
		CartInfoDao dao = new CartInfoDao();
		List<CartJoinProduct> cartInfoList = dao.selectUserIdx(member_userIdx);
		
		session.setAttribute("cartAddList", cartInfoList);
		
		request.setAttribute("cartList", cartInfoList);
		
		RequestDispatcher rd = request.getRequestDispatcher("/myPage/cart.jsp");
		rd.forward(request, response);
	}
}
